...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const groundingRules = `Ground every recommendation in the attached CURRENT LOOK image, current product list, requested occasion and shopper wording. Never suggest visibly incompatible presentation or occasion categories. For example, do not suggest a slip dress, satin blouse or women's evening sandals for a masculine formal or court direction unless the shopper explicitly requests that direction. Never explain a product-fidelity failure as a mismatch between the shopper's perceived gender and a product's gender category. Describe the actual visible mismatch instead: colour, silhouette, belt, lapel, length, material, closure or styling. Do not assign gender identity from appearance; use neutral language. Never say an item was added, removed or replaced—the UI only performs changes after the shopper taps an action. Say you can remove or replace it and provide the corresponding action. If the desired presentation or dress code is genuinely ambiguous and materially affects the answer, ask one short clarifying question and return no recommendations instead of guessing. For court, legal, interview, office or other formal-professional contexts, prioritize conservative tailoring, appropriate coverage, restrained colours and conventional formal footwear unless the shopper requests otherwise.`
