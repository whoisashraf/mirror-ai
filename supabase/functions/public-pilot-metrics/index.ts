...
async function allEvents(service: any, since: string) {
  ...
  for (let from = 0; ; from += 1000) {
    const { data, error } = await service.from('analytics_events')
      .select('id,event_type,session_id,generation_id,created_at')
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const summary = summarizeCommerceFunnel(events)
    ...
    const payload = {
      ...
      tryOns:summary.completedLooks,
      conversations:sessions('chat_started').size,
      checkoutClicks:summary.checkoutSessions,
      tryOnCompletionRate:summary.tryOnCompletionRate,
