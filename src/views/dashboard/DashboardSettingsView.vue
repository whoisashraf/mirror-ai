...
import { getOwnedMerchant, updateMerchant, uploadMerchantAsset } from '@/lib/api'
...
async function uploadAsset(kind:'logo'|'hero',file?:File){if(!merchant.value||!file)return;uploading.value=kind;error.value='';try{const url=await uploadMerchantAsset(file,merchant.value.id,kind);if(kind==='logo')merchant.value.storefront_config!.logoUrl=url;else merchant.value.storefront_config!.heroImageUrl=url}catch(caught){error.value=caught instanceof Error?caught.message:'Asset could not be uploaded.'}finally{uploading.value=''}}
</script>
<template>
  <div class="min-h-screen bg-paper lg:flex"><DashboardNav/><main class="flex-1 p-4 sm:p-6 lg:p-9"><div v-if="loading" class="mx-auto max-w-6xl rounded-2xl border border-line bg-white p-6 text-sm text-muted">Loading store settings…</div><div v-else-if="merchant" class="mx-auto max-w-6xl">
    ...
    <div class="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <div class="rounded-[1.4rem] border border-line bg-white p-5"><div class="grid gap-5">
        ...
        <label class="text-xs font-medium">Custom domain<input v-model="merchant.custom_domain" placeholder="tryon.yourstore.com" class="mt-2 min-h-12 w-full rounded-xl border border-line px-3"/><span class="mt-1 block font-normal text-muted">Save the hostname here after its DNS points to the Mirror deployment. Domain verification and TLS remain a deployment step.</span></label>
        ...
        <div class="border-t border-line pt-5"><h2 class="font-semibold">Storefront branding</h2><div class="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <label class="text-xs font-medium">Logo image<input v-model="merchant.storefront_config!.logoUrl" type="url" placeholder="Paste a URL or upload below" class="mt-2 min-h-12 w-full rounded-xl border border-line px-3"/><span class="mt-2 block"><label class="inline-block cursor-pointer rounded-full border border-line px-3 py-2 font-semibold"><input type="file" accept="image/*" class="hidden" @change="uploadAsset('logo',($event.target as HTMLInputElement).files?.[0])"/>{{uploading==='logo'?'Uploading…':'Upload logo'}}</label></span></label>
          <label class="text-xs font-medium">Hero image<input v-model="merchant.storefront_config!.heroImageUrl" type="url" placeholder="Paste a URL or upload below" class="mt-2 min-h-12 w-full rounded-xl border border-line px-3"/><span class="mt-2 block"><label class="inline-block cursor-pointer rounded-full border border-line px-3 py-2 font-semibold"><input type="file" accept="image/*" class="hidden" @change="uploadAsset('hero',($event.target as HTMLInputElement).files?.[0])"/>{{uploading==='hero'?'Uploading…':'Upload hero image'}}</label></span></label>
          <label class="text-xs font-medium">Accent colour<div class="mt-2 flex gap-2"><input v-model="merchant.storefront_config!.accentColor" type="color" class="h-12 w-14 rounded-xl border border-line bg-white p-1"/><input v-model="merchant.storefront_config!.accentColor" placeholder="#111111" class="min-h-12 min-w-0 flex-1 rounded-xl border border-line px-3"/></div></label>
          <label class="text-xs font-medium">Background colour<div class="mt-2 flex gap-2"><input v-model="merchant.storefront_config!.backgroundColor" type="color" class="h-12 w-14 rounded-xl border border-line bg-white p-1"/><input v-model="merchant.storefront_config!.backgroundColor" placeholder="#f7f5f1" class="min-h-12 min-w-0 flex-1 rounded-xl border border-line px-3"/></div></label>
      ...
      <aside class="xl:sticky xl:top-8 xl:self-start"><p class="mb-3 text-xs font-semibold uppercase tracking-[.18em] text-muted">Preview before saving</p><div class="overflow-hidden rounded-[1.6rem] border border-line shadow-sm" :style="{background:cfg.backgroundColor || '#f7f5f1',color:cfg.textColor || '#111111'}">
