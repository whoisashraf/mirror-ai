...
onMounted(async()=>{ merchant.value=await getOwnedMerchant(); if(merchant.value&&!merchant.value.storefront_config) merchant.value.storefront_config={} })
async function save(){ if(!merchant.value)return; merchant.value=await updateMerchant(merchant.value); saved.value=true; setTimeout(()=>saved.value=false,1800) }
</script>
<template>
  <div class="min-h-screen bg-paper lg:flex"><DashboardNav/><main class="flex-1 p-4 sm:p-6 lg:p-9"><div v-if="merchant" class="mx-auto max-w-6xl">
    ...
    <div class="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <div class="rounded-[1.4rem] border border-line bg-white p-5"><div class="grid gap-5">
        ...
        <button class="min-h-12 rounded-full bg-ink px-5 text-sm font-semibold text-white" @click="save">{{saved?'Saved ✓':'Save settings'}}</button>
      </div></div>
