...
function exportCsv(){if(!data.value)return;const rows=[['Stage','Unique sessions','Conversion from prior stage'],...data.value.funnel.map(step=>[step.label,String(step.value),step.conversionFromPrevious===null?'':`${step.conversionFromPrevious}%`])];const csv=rows.map(row=>row.map(value=>`"${String(value).replaceAll('"','""')}"`).join(',')).join('\n');const url=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));const link=document.createElement('a');link.href=url;link.download=`mirror-pilot-${days.value}-days.csv`;link.click();URL.revokeObjectURL(url)}
...
<template>
  <div class="min-h-screen bg-paper lg:flex"><DashboardNav/><main class="flex-1 p-4 sm:p-6 lg:p-9"><div class="mx-auto max-w-5xl">
    ...
    <div v-else-if="!merchantId" class="mt-7 rounded-2xl border border-line bg-white p-6"><h2 class="font-semibold">Analytics unavailable</h2><p class="mt-2 text-sm text-muted">Sign in with the merchant account that owns this store.</p></div>
